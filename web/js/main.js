...
$('.dot').click((e) => {
	let $target = $(e.target);
	let uid = $target.attr('value');
	var imageEl = document.querySelectorAll(".mySlides.fade");
	$(imageEl[uid]).show().delay(3000);
	$(imageEl).not(imageEl[uid]).hide().delay(3000); //클릭시 사진 더 보여줌
})
